...
///< Initialization messages + concluding messages use syslog or printf
//#define SYSLOG_ALL
#define PRINTF_ALL
...
///< All RT threads will run on this core
#define RT_CORE 2
...
///< POSIX scheduling parameters to create scheduling attributes with
...
void print_scheduler(void)
{
    ...
    case SCHED_RR:
        ...
        break;
        //case SCHED_DEADLINE:
        //    syslog(LOG_INFO, "Pthread Policy is SCHED_DEADLINE\n"); exit(-1);
        //    break;
    default:
...
int main(int argc, char **argv)
{
    ...
    #elif SYSLOG_ALL
    rc = sched_setscheduler(getpid(), SCHED_FIFO, &main_param);
    if (rc < 0) syslog(LOG_ERR, "main_param");
    #endif
    print_scheduler();
    ...
    for (i = 0; i < NUM_OF_THREADS; i++) {
        ...
        if (rc < 0) printf("Failed to initialize rt_sched_attr[%d]\n", i);
        #elif SYSLOG_ALL
        rc = pthread_attr_init(&rt_sched_attr[i]);
        if (rc < 0) syslog(LOG_ERR, "Failed to initialize rt_sched_attr[%d]\n", i);
        #endif

        ///< Set scheduling policies for this thread i
        #ifdef PRINTF_ALL
        rc = pthread_attr_setinheritsched(&rt_sched_attr[i], PTHREAD_EXPLICIT_SCHED);
        if (rc < 0) printf("Failed to set inherit sched for rt_sched_attr[%d]\n", i);
        rc = pthread_attr_setschedpolicy(&rt_sched_attr[i], SCHED_FIFO);
        if (rc < 0) printf("Failed to set SCHED_FIFO policy for rt_sched_attr[%d]\n", i);
        #elif SYSLOG_ALL
        rc = pthread_attr_setinheritsched(&rt_sched_attr[i], PTHREAD_EXPLICIT_SCHED);
        if (rc < 0) syslog(LOG_ERR, "Failed to set inherit sched for rt_sched_attr[%d]\n", i);
        rc = pthread_attr_setschedpolicy(&rt_sched_attr[i], SCHED_FIFO);
        if (rc < 0) syslog(LOG_ERR, "Failed to set SCHED_FIFO policy for rt_sched_attr[%d]\n", i);
        #endif

        ///< Set core affinity for this thread i
        #ifdef PRINTF_ALL
        rc = pthread_attr_setaffinity_np(&rt_sched_attr[i], sizeof(cpu_set_t), &threadcpu);
        if (rc < 0) printf("Failed to set core affinity for rt_sched_attr[%d]\n", i);
        #elif SYSLOG_ALL
        rc = pthread_attr_setaffinity_np(&rt_sched_attr[i], sizeof(cpu_set_t), &threadcpu);
        if (rc < 0) syslog(LOG_ERR, "Failed to set core affinity for rt_sched_attr[%d]\n", i);
        #endif
        ...
        if (rc < 0) printf("Failed to set scheduling priority for rt_sched_attr[%d]\n", i);
        #elif SYSLOG_ALL
        rt_param[i].sched_priority = rt_max_prio - i;
        rc = pthread_attr_setschedparam(&rt_sched_attr[i], &rt_param[i]);
        if (rc < 0) syslog(LOG_ERR, "Failed to set scheduling priority for rt_sched_attr[%d]\n", i);
        #endif

        ///< Store thread IDs based on values of i
    ...
    ///< create frame acquisition thread
    ///< create frame difference threshold thread
    ///< create frame select thread
    ///< create frame process thread
    ///< create frame write-back thread
